...
const AdminReport: React.FC<AdminReportProps> = ({ orders, expenses }) => {
          ...
          <div className="flex flex-col gap-1 w-full sm:w-auto relative">
            ...
            <div className="flex gap-2">
              <input 
                ...
                className="bg-gray-50 border-2 border-transparent focus:border-[#C2A383] rounded-2xl px-6 py-3 font-black text-sm text-[#4B3621] outline-none transition-all cursor-pointer"
              />
              <button 
                ...
                <tr key={idx} className="group hover:bg-gray-50 transition-all">
                  ...
                  <td className="px-6 py-7">
                    <span className="text-[15px] font-black text-[#4B3621]">
                      {item.type === 'IN' ? (item.tableId === 'MANG_VE' ? 'Khách lẻ mang về' : `Phục vụ tại bàn ${item.tableId}`) : item.description}
                    </span>
                  </td>
